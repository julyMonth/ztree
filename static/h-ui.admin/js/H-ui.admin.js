...
if(window.localStorage.token){
		...
		var menu = JSON.parse(window.localStorage.list)
//		console.log(menu);
		let res =menu;
		for(var i = 0; i < res.length; i++) {
			if(res[i].name) {
				var str = '<dl><dt><i class="Hui-iconfont">&#xe616;</i> ' +
					res[i].name +
...
//			} 
//			else {
//				var str1 = '<dl class="secondMenu"><dt><ul><li><a style="border-bottom:none" data-href="' + res[i].child[0].url + '" data-title="' + res[i].child[0].title + '" href="javascript:void(0)">' + res[i].child[0].title + '</a></li></ul></dt></dl>';
//				$('.menu_dropdown').append(str1);
			}
		};
   }else{
		alert('尚未登录，请登录');
	...
	$.ajax({
		type: 'post',
		url: 'http://192.168.1.122:8080/epark/api/v1/logout',
		headers: {"token": token},
